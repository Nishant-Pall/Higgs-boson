...
class NetworkLayer(object):
    """Base class for representing different possible types of neural network layers.
    ...
    def output(self, X):
        """
        Passes the input values 'X' through the layer of the network.
        """
        pass
    ...
    def getWeightsFlat(self):
        """
        Returns the array of weigts in the layer
        """
        return np.array([])

    def setWeights(self, weights):
        """
        Resets the weight vector/matrix.
        """
        pass
...
class LinearLayer(NetworkLayer):
    ...
    def __init__(self, inputShape, outputShape, name='Linear Layer'):
        ...
        self.weights = 0.1 * np.random.randn(inputShape, outputShape)
        self.biases = np.zeros(outputShape)

    def output(self, X):
        """
        Calculates the dot product of weights and inputs + biases.
        """
        return X.dot(self.weights) + self.biases

    def size(self):
        """
        Returns the total number of weights in the layer.
        """
...
class SigmoidLayer(NetworkLayer):
    ...
    def output(self, X):
        """
        Calculates sigmoid function value for each element in the
        input 'X'.
        """
        return 1. / (1. + np.exp(-X))
...
class Neuron(NetworkLayer):
    ...
    def output(self, X):
        return self.sigmoid.output(self.linear.output(X))
    
    def size(self):
        return self.linear.size()

    def getWeightsFlat(self):
        return self.linear.getWeightsFlat()
...
class FunctionLayer(NetworkLayer):
    ...
    def output(self, X):
        """
        Applies the given function (Note: the function has to be vectorized for
